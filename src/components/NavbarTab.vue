    ...
    >
      <div class="flex items-center justify-between">
        <div class="flex">
          ...
          <div class="mx-4">
            <p class="p-3"><button @click="admin()">Admin</button></p>
          </div>
        </div>
        <div class="flex">
          <div class="mx-4">
...
export default {
  data() {
    // กำหนดค่าของรูปภาพของข้อมูลและ status ของการโชว์ email ของผู้ที่ล็อคอินเข้ามา
    return {
      loginStatus: 0,
      loginOnline: 0,
      image: "",
  ...
  methods: {
    ...
    // email koonwinner.ch.lnw@gmail.com password Ch5108186 จะเป็น email ของแอดมิน
    admin() {
      onAuthStateChanged(auth, (user) => {
        if (user.uid == "iqRPAA0tmmYmkkeDJ8Fs0jMtf3R2") {
          const uid = user.uid;
          console.log(uid); 
